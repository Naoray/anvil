...
func NewLaravelSharedDB(siteDrivers ...config.SiteDriver) *LaravelSharedDB {
	...
	return &LaravelSharedDB{
		basePreset: basePreset{
			...
			defaultSteps: append([]config.StepConfig{
				...
				{Name: "php.laravel", Args: []string{"key:generate", "--no-interaction", "--no-ansi"}, ConditionHolder: config.ConditionHolder{Condition: map[string]any{"env_file_missing": "APP_KEY"}}},
			...
			cleanupSteps: siteCleanupSteps(siteDriver),
		},
	}
}
